...
export const loginUserHandler = handler(async (req, res) => {
  ...
  (await findUserByEmail(body.email))
    .mapErr(err => {
      switch (err) {
        case "notFound":
          return res.status(StatusCodes.BAD_REQUEST).json("Email or password is invalid")
        case "unknown":
          return res.sendStatus(StatusCodes.INTERNAL_SERVER_ERROR)
      }
    })
    .map(async foundUser => {
      const isCorrectPassword = await compare(body.password, foundUser.hashedPassword)
